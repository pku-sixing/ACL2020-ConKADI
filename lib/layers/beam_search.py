...
class BeamSearchDecoderMixin(object):
  ...
  def __init__(self,
    ...
    self._diverse_decoding_rate = diverse_decoding_rate
  ...
  def step(self, time, inputs, state, name=None):
    ...
    with ops.name_scope(name, "BeamSearchDecoderStep", (time, inputs, state)):
      ...
      beam_search_output, beam_search_state = _beam_search_step(
          ...
          diverse_decoding_rate = self._diverse_decoding_rate,
      )
...
class BeamSearchDecoder(BeamSearchDecoderMixin, decoder.Decoder):
  ...
  def __init__(self,
    ...
    super(BeamSearchDecoder, self).__init__(
        ...
        diverse_decoding_rate=diverse_decoding_rate,
        length_penalty_weight=length_penalty_weight,
        coverage_penalty_weight=coverage_penalty_weight,
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
                      ...
                      coverage_penalty_weight,diverse_decoding_rate):
  ...
  def diverse_decoding_offset(beam_prob, rate):
      argsort_index = tf.argsort(beam_prob, -1, direction='DESCENDING')
      argsort_index = tf.argsort(argsort_index, -1)  # Rank
      to_apply_rate = tf.where(tf.less(time, 1), 0.0, rate)
      rankaware_offset = tf.to_float(argsort_index) * - to_apply_rate
      return beam_prob + rankaware_offset

  if diverse_decoding_rate > 0.0:
      print('Enabled Diverse Decoding')
      scores = diverse_decoding_offset(scores, diverse_decoding_rate)
